...
import heroVideo from '../assets/hero video/Developer_introduces_self_and_sk.mp4';
...
const Hero = () => {
  ...
  useEffect(() => {
    ...
    // Auto-play the muted background video
    if (videoRef.current) {
  ...
  return (
    <section className="relative w-full h-screen overflow-hidden bg-black">
      {/* Background Video with Dark Theme Vignette */}
      <div className="absolute top-0 right-0 w-full md:w-[55%] h-full z-0 overflow-hidden">
        <video
          ...
          muted={isMuted}
          playsInline
          className="w-full h-full object-cover opacity-45 object-[center_25%]"
        >
          <source src={heroVideo} type="video/mp4" />
          Your browser does not support the video tag.
        </video>
        {/* Gradients to fade to black on the left, top, and bottom */}
      ...
      <div className="absolute inset-0 z-20 px-6 pb-20 md:pb-[8%] md:px-12 max-w-7xl mx-auto flex flex-col md:flex-row justify-end md:justify-between items-start md:items-end text-left w-full">
        ...
        <div className="flex flex-col items-start text-left max-w-2xl w-full">
          ...
          >
            Hi, I’m <br /> <span className="text-white">Ajay Gopagoni</span>
          </h1>
          ...
          <p 
            data-aos="fade-up"
            data-aos-delay="200"
            className="text-white/80 text-sm md:text-lg font-medium mb-8 max-w-md drop-shadow-md"
          >
            I build intelligent models, deep learning systems, and data-driven applications specializing in AI & Machine Learning.
          </p>
          ...
          >
            {/* Primary Button */}
            <a href="#projects" className="px-4 py-2 md:px-6 md:py-2 text-xs md:text-base rounded-full bg-white text-black font-semibold hover:bg-gray-200 transition-all duration-300 transform hover:scale-105 shadow-md text-center">
              View My Work
            </a>
